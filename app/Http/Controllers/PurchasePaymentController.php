...
class PurchasePaymentController extends Controller
{
    ...
    public function upload(Request $r)
    {
        ...
        Log::info("Upload year={$yearToUse}");
        ...
        foreach($rows as $i=>$row){
            try{
                $data=[]; foreach($header as $k=>$v) $data[$v]=$row[$k]??null;
                $cols=[
                    ...
                    'Cluster'=>$data['Cluster']??null,'Block'=>$data['Block']??null,'Unit'=>$data['Unit']??null,'CustomerName'=>$data['CustomerName']??null,
                    'PurchaseDate'=>$this->parseDate($data['PurchaseDate']??null),'LunasDate'=>$this->parseDate($data['LunasDate']??null),
                    'is_ppndtp'=>$this->toFloat($data['is_ppndtp']??null),'persen_ppndtp'=>$this->toFloat($data['persen_ppndtp']??null),
                    'harga_netto'=>$this->toFloat($data['harga_netto']??null),'TotalPPN'=>$this->toFloat($data['TotalPPN']??null),
                    'harga_bbnsertifikat'=>$this->toFloat($data['harga_bbnsertifikat']??null),'harga_bajb'=>$this->toFloat($data['harga_bajb']??null),
                    'harga_bphtb'=>$this->toFloat($data['harga_bphtb']??null),'harga_administrasi'=>$this->toFloat($data['harga_administrasi']??null),
                    'harga_paket_tambahan'=>$this->toFloat($data['harga_paket_tambahan']??null),'harga_admsubsidi'=>$this->toFloat($data['harga_admsubsidi']??null),
                    'biaya_asuransi'=>$this->toFloat($data['biaya_asuransi']??null),'HrgJualTotal'=>$this->toFloat($data['HrgJualTotal']??null),
                    'disc_collection'=>$this->toFloat($data['disc_collection']??null),'HrgJualTotalminDiscColl'=>$this->toFloat($data['HrgJualTotalminDiscColl']??null),
                    'TypePembelian'=>$data['TypePembelian']??null,'bank_induk'=>$data['bank_induk']??null,'KPP'=>$data['KPP']??null,
                    'JenisKPR'=>$data['JenisKPR']??null,'Member'=>$data['Member']??null,'Salesman'=>$data['Salesman']??null,
                    'tanggal_akad'=>$this->parseDate($data['tanggal_akad']??null),'persen_progress_bangun'=>$this->toFloat($data['persen_progress_bangun']??null),
                    'type_unit'=>$data['type_unit']??null,'selisih'=>$this->toFloat($data['selisih']??null),
                    'dari_1_sampai_30_DP'=>$this->toFloat($data['dari_1_sampai_30_DP']??null),'dari_31_sampai_60_DP'=>$this->toFloat($data['dari_31_sampai_60_DP']??null),
                    'dari_61_sampai_90_DP'=>$this->toFloat($data['dari_61_sampai_90_DP']??null),'diatas_90_DP'=>$this->toFloat($data['diatas_90_DP']??null),
                    'lebih_bayar'=>$this->toFloat($data['lebih_bayar']??null),'data_year'=>$yearToUse,'data_month'=>$month,'project_id'=>$project,
                    'created_at'=>now(),'created_by'=>auth()->user()->id??null
                ];
                ...
                foreach(array_keys($cols) as $col){
                    if(!in_array($col,$existing)){
                        try{
                            $type=str_contains($col,'Date')?'datetime2':(str_contains($col,'Type')||in_array($col,['Cluster','Block','Unit','CustomerName','TypePembelian','bank_induk','KPP','JenisKPR','Member','Salesman','type_unit'])?'nvarchar(255)':(in_array($col,['data_year','data_month','project_id'])?'int':'decimal(20,2)'));
                            DB::connection('sqlsrv')->statement("ALTER TABLE purchase_payments ADD [{$col}] {$type} NULL");
                        }catch(\Exception $e){Log::warning("Column {$col}: ".$e->getMessage());}
                    }
                }

                DB::connection('sqlsrv')->table('purchase_payments')->updateOrInsert($cols);
                $ok++;
            }catch(\Exception $e){$fail++;Log::error("Row ".($i+2).": ".$e->getMessage());}
        }

                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('payments.upload') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-text">
                                Select the year of the data you're uploading.
                            </div>
                            @error('data_year')
                        ...
                        <div class="mb-3">
                            <label for="data_month" class="form-label">Data Month <span class="text-danger">*</span></label>
                            <select name="data_month" id="data_month" class="form-select @error('data_month') is-invalid @enderror" required>
                                ...
                                @foreach($months as $num => $name)
                                    <option value="{{ $num }}" {{ old('data_month', date('n')) == $num ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="form-text">
                                Select the last month included in your data.
                            </div>
                            @error('data_month')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
